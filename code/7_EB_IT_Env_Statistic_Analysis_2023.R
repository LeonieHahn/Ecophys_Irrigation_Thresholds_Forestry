# =============================================================================
# Analyse how the irrigation demand of F. sylvatica and Q. robur on EB in 2023
# relates to environmental drought conditions
# (Note: only unirrigated treatment is used here)
# - Calculate rolling statistics for the VPD over different timesteps
# - Calculate Correlations between the irrigation demand and environmental 
...
# for different timesteps (2-7 days)
columns <- c("VPD_kPA_mean", "VPD_kPA_min", "VPD_kPA_max")
windows <- c(seq(2, 7, 1)) 
funcs <- list(mean = mean, max = max, min = min) 

irrig_thres_EB <- calculate_rolling_stats(irrig_thres_EB, columns, 
                                          windows, funcs)
...
treatment_list <- c("Unirrigated", "Overall") 
    ...
    if (species == "Overall" & treatment == "Overall") {
      correlation_data <- irrig_thres_EB %>%
        filter(Tree.Species %in% c("Quercus robur", "Fagus sylvatica"),
               Treatment %in% c("Unirrigated")) %>% 
        calculate_all_correlations_EB_2023(.)
...
write.csv(final_cor_matrix, 
          "./data/EB/results/Irrigation_thresholds/Cor_EB_2023_all.csv")
write.csv(final_p_matrix, 
  "./data/EB/results/Irrigation_thresholds/Cor_EB_2023_all_p_value.csv")
...
# Part 1: Fagus: Precip_mm_10min_sum_2_mean,  VPD_kPA_mean
# Part 2: Quercus: Precip_mm_10min_sum_2_mean, VPD_kPA_mean_2_min
# Part 3: Overall: Precip_mm_10min_sum_2_mean, VPD_kPA_mean_2_min

part_1 <- irrig_thres_EB %>% 
  filter(!is.na(Precip_mm_10min_sum_2_mean) & !is.na(VPD_kPA_mean) &
           !is.na(SWC_mean))

# calculate effect of drought variables on the irrigation demand 
# for F. sylvatica
species_results_part_1 <- 
  evaluate_irrigation_models_by_species_part_1_2023(part_1)
species_results_part_1_sel <- filter(species_results_part_1, 
                                     Tree.Species == "Fagus sylvatica")

# Part 2: Quercus: Precip_mm_10min_sum_2_mean, VPD_kPA_mean_2_min
part_2 <- irrig_thres_EB %>%
  filter(!is.na(Precip_mm_10min_sum_2_mean) & !is.na(VPD_kPA_mean_2_min))

# calculate effect of drought variables on the irrigationd demand 
# for Q. robur
species_results_part_2 <- 
  evaluate_irrigation_models_by_species_and_treatment_part_2_2023(part_2)
species_results_part_2_sel <- filter(species_results_part_2, 
                                     Tree.Species == "Quercus robur")


# Part 3: Precip_mm_10min_sum_2_mean, VPD_kPA_mean_2_min
part_3 <- irrig_thres_EB %>%
  filter(!is.na(Precip_mm_10min_sum_2_mean) & !is.na(VPD_kPA_mean_2_min))

# calculate effect of drought variables on the irrigationd demand 
# for both species combined
species_treatment_results_part_3 <- 
  evaluate_irrigation_models_overall_part_3_2023(part_3)

# combine all results
mr_results_sel <- rbind(species_results_part_2_sel,
                        species_treatment_results_part_3)

mr_results_sel <- full_join(mr_results_sel, species_results_part_1_sel)
...
mr_results_sel <- mr_results_sel %>%
  ...
  mutate(across(
    .cols = all_of(names(mr_results_sel)[startsWith(names(mr_results_sel), 
                                                    "p_")]),  
    .fns = ~ case_when(
      .x < 0.001 ~ "< 0.001",
      .x < 0.01  ~ "< 0.01",
...
write.csv(mr_results_sel,
          "./data/EB/results/Irrigation_thresholds/Part_Reg_EB_2023.csv",
          row.names = FALSE)
